...
if( $sql = $conn -> query("SELECT * FROM users WHERE ID = $userID") ){
    ...
    //if there is active match show the user's profile
    if ($findMatchedInterests->num_rows > 0) {
        ...
        while ($data = mysqli_fetch_assoc($sql)) {
            ...
            echo "<p style='color: #0066cc; clear: both;'>
                           <strong>Matched interests:&nbsp;</strong><p style='color: #0066cc; class='commonInterests'>$matchedInterests</p>
                        </p>
                        </div>
